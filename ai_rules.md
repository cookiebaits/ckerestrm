...
* **P3 - Excessive Prompting:** The only time prompting should happen is if its a crucial decision that may affect the entire program (ex: Should we use python vs Node.js). To simplify, if, for example "Is there anything else you would like me to adjust or implement before I finalize and deliver these changes?" never prompt, it is always yes and always proceed to finishing.
