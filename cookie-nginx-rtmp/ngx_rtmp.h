...
{
    if (in->buf->pos + 1 >= in->buf->last) {
        return 0;
    }

    /* Enhanced RTMP packet header check */
    if (in->buf->pos[0] & 0x80) {
        /* IsExHeader is true */
        if (in->buf->pos + 5 >= in->buf->last) {
            return 0;
        }
        /* PacketType is SequenceStart (0) */
        return (in->buf->pos[0] & 0x0f) == 0;
    }

    /* Traditional RTMP codec header check (AVCPacketType == 0 or AACPacketType == 0) */
    return in->buf->pos[1] == 0;
}
