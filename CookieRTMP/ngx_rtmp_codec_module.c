        ...
        ngx_chain_t *in)
{
    ...
    if (h->type == NGX_RTMP_MSG_AUDIO) {
        if (fmt & 0x80) {
            /* Enhanced RTMP Audio ExHeader */
            if (in->buf->last - in->buf->pos < 5) {
                return NGX_OK;
            }
            if (ngx_memcmp(in->buf->pos + 1, "Opus", 4) == 0) {
                ctx->audio_codec_id = NGX_RTMP_AUDIO_OPUS;
            }
        } else {
        ...
        if (fmt & 0x80) {
            /* Enhanced RTMP Video ExHeader */
            if (in->buf->last - in->buf->pos < 5) {
                return NGX_OK;
            }
            if (ngx_memcmp(in->buf->pos + 1, "hvc1", 4) == 0 ||
                ngx_memcmp(in->buf->pos + 1, "hev1", 4) == 0)
            {
                ctx->video_codec_id = NGX_RTMP_VIDEO_HEVC;
            } else if (ngx_memcmp(in->buf->pos + 1, "av01", 4) == 0) {
                ctx->video_codec_id = NGX_RTMP_VIDEO_AV1;
            } else if (ngx_memcmp(in->buf->pos + 1, "vp09", 4) == 0) {
...
ngx_rtmp_codec_parse_avc_header(ngx_rtmp_session_t *s, ngx_chain_t *in)
{
    ...
    /* sanity check dimensions to prevent overflow (support up to 8K) */
...
static void
ngx_rtmp_codec_parse_hevc_sps(ngx_rtmp_session_t *s, u_char *pos, u_char *last)
{
    ngx_rtmp_bit_reader_t   br;
    ngx_uint_t              n, max_sub_layers_minus1, width, height;
    ngx_uint_t              sub_layer_profile_present_flag[8];
    ngx_uint_t              sub_layer_level_present_flag[8];
    ngx_rtmp_codec_ctx_t   *ctx;

    ctx = ngx_rtmp_get_module_ctx(s, ngx_rtmp_codec_module);

    ngx_rtmp_bit_init_reader(&br, pos, last);

    ngx_rtmp_bit_read(&br, 4); /* sps_video_parameter_set_id */
    max_sub_layers_minus1 = (ngx_uint_t) ngx_rtmp_bit_read(&br, 3);
    ngx_rtmp_bit_read(&br, 1); /* sps_temporal_id_nesting_flag */

    /* profile_tier_level(1, max_sub_layers_minus1) */
    ngx_rtmp_bit_read(&br, 2);  /* general_profile_space */
    ngx_rtmp_bit_read(&br, 1);  /* general_tier_flag */
    ngx_rtmp_bit_read(&br, 5);  /* general_profile_idc */
    ngx_rtmp_bit_read(&br, 32); /* general_profile_compatibility_flags */
    ngx_rtmp_bit_read(&br, 48); /* general_constraint_indicator_flags */
    ngx_rtmp_bit_read(&br, 8);  /* general_level_idc */

    for (n = 0; n < max_sub_layers_minus1; n++) {
        sub_layer_profile_present_flag[n] = (ngx_uint_t) ngx_rtmp_bit_read(&br, 1);
        sub_layer_level_present_flag[n]   = (ngx_uint_t) ngx_rtmp_bit_read(&br, 1);
    }

    if (max_sub_layers_minus1 > 0) {
        for (n = max_sub_layers_minus1; n < 8; n++) {
            ngx_rtmp_bit_read(&br, 2);
        }
    }

    for (n = 0; n < max_sub_layers_minus1; n++) {
        if (sub_layer_profile_present_flag[n]) {
            ngx_rtmp_bit_read(&br, 88);
        }
        if (sub_layer_level_present_flag[n]) {
            ngx_rtmp_bit_read(&br, 8);
        }
    }

    ngx_rtmp_bit_read_golomb(&br); /* sps_seq_parameter_set_id */
    if (ngx_rtmp_bit_read_golomb(&br) == 3) { /* chroma_format_idc */
        ngx_rtmp_bit_read(&br, 1); /* separate_colour_plane_flag */
    }
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_RTMP, s->connection->log, 0,
                   "codec: hevc dimensions %ui x %ui", width, height);
}
...
{
    ngx_uint_t              n, m, num_arrays, num_nalus, nal_size, nal_type;
    ngx_rtmp_codec_ctx_t   *ctx;
    ngx_rtmp_bit_reader_t   br;
    ...
    /* Enhanced RTMP ExHeader: 5 bytes (IsExHeader, FrameType, PacketType, FourCC) */
    if (in->buf->pos[0] & 0x80) {
        ngx_rtmp_bit_read(&br, 40);
    } else {
        /* Traditional RTMP HEVC header (PacketType + hvcC) */
        ngx_rtmp_bit_read(&br, 16);
    }
    ...
    /* hvcC (HEVC Configuration Record) parsing */
    ngx_rtmp_bit_read(&br, 8);  /* configurationVersion */
    ngx_rtmp_bit_read(&br, 8);  /* profile_space etc */
    ngx_rtmp_bit_read(&br, 32); /* profile_idc etc */
    ngx_rtmp_bit_read(&br, 48); /* constraint_indicator_flags */
    ngx_rtmp_bit_read(&br, 8);  /* level_idc */
    ngx_rtmp_bit_read(&br, 48); /* min_spatial_segmentation_idc etc */
    ngx_rtmp_bit_read(&br, 8);  /* parallelismType */
    ngx_rtmp_bit_read(&br, 8);  /* chromaFormat */
    ngx_rtmp_bit_read(&br, 8);  /* bitDepthLumaMinus8 */
    ngx_rtmp_bit_read(&br, 8);  /* bitDepthChromaMinus8 */
    ngx_rtmp_bit_read(&br, 16); /* avgFrameRate */
    ngx_rtmp_bit_read(&br, 8);  /* constantFrameRate etc */

    num_arrays = (ngx_uint_t) ngx_rtmp_bit_read(&br, 8);

    for (n = 0; n < num_arrays; n++) {
        ngx_rtmp_bit_read(&br, 1); /* array_completeness */
        ngx_rtmp_bit_read(&br, 1); /* reserved */
        nal_type = (ngx_uint_t) ngx_rtmp_bit_read(&br, 6);
        num_nalus = (ngx_uint_t) ngx_rtmp_bit_read(&br, 16);
        for (m = 0; m < num_nalus; m++) {
            nal_size = (ngx_uint_t) ngx_rtmp_bit_read(&br, 16);
            if (nal_type == 33) { /* SPS */
                ngx_rtmp_codec_parse_hevc_sps(s, br.pos, br.pos + nal_size);
            }
            ngx_rtmp_bit_read(&br, nal_size * 8);
        }
        if (br.err) {
            return;
        }
...
ngx_rtmp_codec_reconstruct_meta(ngx_rtmp_session_t *s)
{
    ...
    v.width = ctx->width;
    v.height = ctx->height;
    v.rotation = (double) ctx->rotation;
        ...
        ngx_chain_t *in)
{
    ...
    ctx->rotation = (ngx_uint_t) v.rotation;
